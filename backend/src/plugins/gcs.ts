...
const gcsPlugin: FastifyPluginAsync = fp(async (fastify) => {
  ...
  fastify.decorate('gcsSignUploadUrl', async (input: SignedUploadUrlInput) => {
    const expiresInSeconds = input.expiresInSeconds ?? defaultExpiresInSeconds;
